...
router.get('/vault-acdc-setup-only', (req, res) => {
  ...
  res.render('paypal/jssdk-v5/vault-acdc-setup-only', {
    ...
    showSidebar:      true,
    sdkUrl: `https://www.paypal.com/sdk/js?client-id=${clientId}&components=card-fields&vault=true`,
...
router.post('/api/vault-acdc-setup-only/create-setup-token', async (req, res) => {
  try {
    ...
    const returnUrl   = `${baseUrl}/paypal/jssdk-v5/vault-acdc-setup-only`
    const cancelUrl   = `${baseUrl}/paypal/jssdk-v5/vault-acdc-setup-only`
...
router.get('/api/vault-acdc-setup-only/setup-token/:setupTokenId', async (req, res) => {
  try {
    const { setupTokenId } = req.params
    const token = await getCNToken()
    const r = await fetch(`${API}/v3/vault/setup-tokens/${setupTokenId}`, {
      method:  'GET',
