/* Custom: ACDC Vault with purchase — card saved on successful payment */
...
const PROVIDER = "paypal",
  SDK = "jssdk-v5",
  KEY = "vault-acdc-with-purchase";

const SCA_METHODS = ["SCA_WHEN_REQUIRED", "SCA_ALWAYS"];

router.get("/vault-acdc-with-purchase", (req, res) => {
  ...
  res.render("paypal/jssdk-v5/vault-acdc-with-purchase", {
    ...
    showSidebar: true,
    sdkUrl: `https://www.paypal.com/sdk/js?client-id=${clientId}&components=card-fields&vault=true&currency=${currency}`,
...
router.post("/api/vault-acdc-with-purchase/create-order", async (req, res) => {
  try {
...
router.get(
  "/api/vault-acdc-with-purchase/order/:orderID",
  async (req, res) => {
    try {
...
router.post(
  "/api/vault-acdc-with-purchase/capture-order",
  async (req, res) => {
    try {
      ...
      const vaultInfo =
        data?.payment_source?.card?.attributes?.vault || null;
