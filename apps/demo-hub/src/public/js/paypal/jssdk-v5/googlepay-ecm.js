...
;(function () {
  ...
  function onGooglePaymentButtonClicked(config) {
    ...
    console.log('[GooglePay ECM] amount:', amount, '| currency:', currency, '| scaMethod:', sca)
    console.log('[GooglePay ECM] shipping:', shipping)

    var createBody = { amount: amount, currency: currency, shipping: shipping, scaMethod: sca }
    ...
    fetch(urls.createOrder, {
      ...
      .then(function (d) {
        console.log('[GooglePay ECM] ===== createOrder response =====')
        console.log('[GooglePay ECM] response:', d)
        ...
        var client = getGooglePaymentsClient()
        console.log('[GooglePay ECM] calling loadPaymentData()...')
        return client.loadPaymentData(paymentDataRequest)
      })
      .then(function (paymentData) {
        // Sheet is now closed — safe to trigger 3DS window
        console.log('[GooglePay ECM] ===== loadPaymentData resolved =====')
        console.log('[GooglePay ECM] paymentData:', paymentData)
        console.log('[GooglePay ECM] paymentData.paymentMethodData:', paymentData.paymentMethodData)
