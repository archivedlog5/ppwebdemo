...
**Files:**
- Create: `src/routes/braintree/server-sdk/dropin-ui.js`
...
```js
// apps/demo-hub/src/routes/braintree/server-sdk/dropin-ui.js
...
```html
<!-- apps/demo-hub/src/views/braintree/server-sdk/dropin-ui.ejs -->
...
<div class="sandbox-page">
  <div class="sandbox-header">
    ...
    <h1><%= title %></h1>
    <p>Pre-built payment UI — card, PayPal, Venmo, Apple Pay, Google Pay in one widget</p>
  </div>
  ...
  <div class="sandbox-card">
    <div class="amount-row">
      ...
      <div class="amount-group">
        ...
        <select id="demo-3ds" class="currency-select">
          ...
          <option value="when_required">SCA When Required</option>
          <option value="always">SCA Always</option>
        </select>
      </div>
    </div>
    ...
    <button id="submit-btn" class="pay-btn" style="display:none">Pay</button>
...
```js
// apps/demo-hub/src/public/js/braintree/server-sdk/dropin-ui.js
;(function () {
  'use strict'
  ...
  function getAmount() {
    ...
    return input ? input.value.trim() : (window.DEMO.defaultAmount || '10.00')
  }
  ...
  window.addEventListener('load', function () {
    ...
    function initDropin() {
      ...
      if (mode !== 'none') {
        options.threeDSecure = { amount: amount }
      }

      braintree.dropin.create(options, function (err, instance) {
    ...
    // Re-init when 3DS mode changes (amount changes are handled at submit)
    document.getElementById('demo-3ds').addEventListener('change', function () {
      if (dropinInstance) {
        dropinInstance.teardown(function () { dropinInstance = null; initDropin() })
      }
    })
    ...
    document.getElementById('submit-btn').addEventListener('click', function () {
      ...
      if (!amount || isNaN(num) || num <= 0) {
        showResult('✗ Invalid amount', 'error')
        btn.disabled = false
        return
      }
      ...
      dropinInstance.requestPaymentMethod(function (err, payload) {
        ...
        fetch(urls.transaction, {
          ...
          body:    JSON.stringify({ nonce: payload.nonce, amount: parseFloat(amount).toFixed(2) }),
        })
...
Visit `http://localhost:3000/braintree/server-sdk/dropin-ui`. Verify:
- Page loads, Drop-in widget renders with payment options
- Enter amount `10.00`, use test card `4111111111111111` exp `12/2025` CVV `123`
- Click Pay → result shows `✓ submitted_for_settlement · Transaction ID: xxx`

- [ ] **Step 5: Commit**

```bash
git add apps/demo-hub/src/routes/braintree/server-sdk/dropin-ui.js \
        apps/demo-hub/src/views/braintree/server-sdk/dropin-ui.ejs \
        apps/demo-hub/src/public/js/braintree/server-sdk/dropin-ui.js
git commit -m "feat(braintree): add dropin-ui demo"
```
